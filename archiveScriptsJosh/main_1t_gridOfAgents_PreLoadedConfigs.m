...
% Performance parameters	
	...
	% Tether radius
	tetherRadiusRange = 7:20;%0:2:20;%0:50:200;%:50:250;

	% max UAV flight velocity
	maxAgentSpeed = 5:2.5:20;

	% Energy threshold + initial energy reserves
...
% Simulation initial parameters
	...
	t_init              = 0;
	num_sims = 1; % repeating sim to account for varying grid configurations
...
for p = 1:length(numAgentsArray)
	...
	% Extract coordinates from excel
	tempSheetName = strcat("Sheet", num2str(noActiveAgents));
	configData = xlsread('successfulConfigs.xlsx',tempSheetName);
	
	% If there are no valid configurations, continue
	if isempty(configData)
		continue
	end
	
	% If the sheet is now proven to contain data, remove the first two rows which correspond
	% to headers
	configData = configData(3:end,:);
	
	
	%activeAgentsCoords = getAgentInitialCoords(noActiveAgents,property_size, buffer_size, sim);
	% Right now it is just extracting the first available config --> should
	% sample from the grids with the biggest range of 'success'
	numTetherOptionsForCurrentGrid = NaN(round(length(configData(:,1))/noActiveAgents),1);
	bestGridsIndices = []; % to store [first agent index in current configuration, number of viable tether options]
	
	for configIndx = 1:noActiveAgents:length(configData(:,1))-1
		agentIndx1 = configIndx;
		agentIndx2 = configIndx + noActiveAgents - 1;
		
		% Extract the number of non NaN tether lengths
		currentConfigTethers = configData(configIndx, 3:end);
		
		% Check for NaNs
		currentConfigTetherNaNs = isnan(currentConfigTethers);
		
		% Viable tether counter
		viableTetherCounter = 0;
		for m = 1:length(currentConfigTetherNaNs)
			if currentConfigTetherNaNs(m) == false
				% This tether length is viable
				viableTetherCounter = viableTetherCounter + 1;
			end
		end
		
		tmpIndx = (configIndx - 1)/noActiveAgents + 1; %  Used for mapping weird indices such as 1,8,15 ==> 1,2,3
		numTetherOptionsForCurrentGrid(tmpIndx) = viableTetherCounter;
		
		% Check if best/equal best option so far - by default first one is
		% already first max
		if configIndx == 1
			% Default first option
			bestGridsIndices = [configIndx, numTetherOptionsForCurrentGrid(tmpIndx)];
		elseif numTetherOptionsForCurrentGrid(tmpIndx) > max(bestGridsIndices(:,2))
			% Reset array with new single maximum
			bestGridsIndices = [configIndx, numTetherOptionsForCurrentGrid(tmpIndx)]; 
		elseif numTetherOptionsForCurrentGrid(tmpIndx) == max(bestGridsIndices)
			% Append the current index value since it is identical to the
			% current maximum
			bestGridsIndices = [bestGridsIndices; configIndx, numTetherOptionsForCurrentGrid(tmpIndx)]; 
		end
	end
	
	% Now, only sample from the 'best' grids (i.e. the one/s with the
	% largest successful tether range) - Assume each grid is equally
	% weighted as only the maxima have been extracted anyway
	bestGridWeights = ones(1,length(bestGridsIndices(:,1)))/length(bestGridsIndices(:,1));
	if length(bestGridsIndices(:,1)) == 1
		% Random sample doesn't work when there is only one element...
		configChoiceAgent1Indx = bestGridsIndices(randsample(length(bestGridsIndices(:,1)),1),1);
	else
		configChoiceAgent1Indx = randsample(bestGridsIndices(:,1),1,true,bestGridWeights);
	end
	
	
	% Clean up first and last indices from the good grid sample
	a_indxFirst = configChoiceAgent1Indx;
	a_indxLast  = configChoiceAgent1Indx + noActiveAgents - 1;
	activeAgentsCoords = configData(a_indxFirst:a_indxLast,1:2); % Only extract x,y coords
