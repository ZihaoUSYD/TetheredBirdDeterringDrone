classdef Aircraft
    ...
    properties
        ...
        pitch                   % pitch angle
        yaw                     % heading angle
		default_yaw				% heading angle at base/upon return
		...
		max_vert_velocity		% maximum vertical velocity of typical quadrotor UAVs
		min_vert_velocity		% minimum vertical velocity of UAV
		...
		% Vertical gain 'sets' - same as above, but set '1' for chasing and
		% set '2' for holding steady state while close to setpoint
		vertPos_p1					
		vertVel_p1
		vertAcl_p1
		vertPos_p2
		...
		% Individual cost function contributions
		tmp_probability_cost
		tmp_distance_cost
		tmp_heading_cost
		tmp_tether_cost
		tmp_repField_cost
		tmp_otherAgent_cost
		tmp_cost % this is the combined cost function scaled to [0 1]
    ...
    methods
        ...
        function obj = Aircraft(type,x,y,z,roll,pitch,yaw,velocity,max_velocity,min_velocity,max_yawrate)
            if nargin > 0
				...
				obj.max_vert_velocity = 5; % m/s, based off DJI Phantom
            ...
            obj.max_acceleration    = 5; % appropriate in both horizontal and vertical dimensions
            ...
            elseif obj.type == 2
                ...
                obj.pos_p       = 0.15;
				obj.vel_p       = 0.7;
				obj.yaw_p       = 0.8; % was 0.7
                obj.yaw_rate_p  = 0.5;
                obj.yaw_acl_p   = 0.6;
                obj.acl_p       = 0.5;
				
				% vertical gains - (1) one set for chasing, (2) one set for holding
				% steady state
				obj.vertPos_p1 = 0.65;
				obj.vertVel_p1 = 0.6;
				obj.vertAcl_p1 = 100;
				
				obj.vertPos_p2	= obj.vertPos_p1;
				obj.vertVel_p2	= obj.vertVel_p1; 
				obj.vertAcl_p2	= obj.vertAcl_p1;

				obj.vertPos_p	= obj.vertPos_p1;
				obj.vertVel_p	= obj.vertVel_p1;
				obj.vertAcl_p	= obj.vertAcl_p1;
        ...
        function [obj] = moveAgent(obj, world, cumulation, dt, other_agents_coordinates,tether_radius, num_targets)
            ...
            % save this to temporary cost function for tether limit
			obj.tmp_tether_cost = unreachable_grids; % these unreachable grids dont account for other agent cost
			...
			elseif obj.type == 2
				obj                 = getMultirotorWaypoint(obj, world, unreachable_grids, grid_agent_yaw, grid_agent_dist, cumulation, other_agents_coordinates, dt);
			end
	...
	%             [y_p_max_index, x_p_max_index]      = ind2sub(size(world.prob_global(:,:,cumulation-1)),prob_index);
				...
				k_max = ceil(0.5*length(uniqueProbabilities)); % take the first 30% of maximum values, if allowable,
...
% 				[smallest_dist_current_agent]		= min(dist_to_maxProb_from_tether, dist_to_max_probability(1));
				...
				% Choose gains based off current position to setpoint position
				% of agent
				distToBase = sqrt((agent_x-tether_x)^2 + (agent_y-tether_y)^2 + ((agent_z-tether_z)^2));
				distToAltitude = abs(obj.z_setpoint(cumulation) - agent_z);
				
				if distToAltitude < 2  % arbitrary distance thresholds
					% Modify gains for steady state attainment (either at
					% base or at the default altitude)
					obj.vertPos_p	= obj.vertPos_p2;
					obj.vertVel_p	= obj.vertPos_p2;
					obj.vertAcl_p	= obj.vertPos_p2;
				else
					% Keep default chase gains
					obj.vertPos_p	= obj.vertPos_p1;
					obj.vertVel_p	= obj.vertPos_p1;
					obj.vertAcl_p	= obj.vertPos_p1;
				end
				...
				if min(dist_to_maxProb_from_tether) <= (tether_radius)% (&& smallest_dist == min(dist_to_max_probability(1,:,:))) && i.e. current agent corresponds to closest agent (and is within appropriate range to the closest target)
					% This allows ANY agent to deploy if the target is
					% WITHIN its tether radius range
				...
				elseif  canReachFlag % && (smallest_dist == dist_to_max_probability(1)) % NOTE: include this if you only want closest agent to move: % (min(dist_to_maxProb_from_tether) > tether_radius) && 
					% This allows ANY agent to deploy if the target is at
					% least within its potential scaring radius (which is
					% determined to be a set distance beyong its maximum
					% tether point)
					...
					obj.yaw_setpoint(cumulation) = obj.default_yaw;
					...
					% Check if agent is already at home base or within
        ...
        % ========================
        function [obj] = getMultirotorWaypoint(obj, world, unreachable_grids, grid_agent_yaw, grid_agent_dist, cumulation, other_agents_coordinates, dt)
            % calculate cost to reach each grid centres
            % weights
            alpha = 0.4; % probability weight % was 0.4
            beta = 0.15;  % distance weight % was 0.1
            gamma = 0.05;%0.375; % heading weight % default was 0.5
			zeta = 0.4; % change in probability field potential weight
			...
			k_max = 1; % max probability score for a cell

			prob_potential_field_cost = zeta*(1+delta_probability)/2*k_max; % max probability is 1, so it's automatically normalised! Note this is mapped from [-1,1] to [0,1]
            ...
            % distance cost
            distance_cost   = beta*grid_agent_dist./world.RHC_steps*dt/obj.max_velocity; %forgot dt
            % heading cost
			...
			if isempty(other_agents_coordinates) == true
				...
				for i = 1:length(other_agents_coordinates(:,1))
					...
					otheragents_grid_temp = (grid_other_agent_dist < (15));
			...
			% Save temporary cost functions at current time step (used in
			% plotting individual contributions); make sure to remove
			% weightings that were applied earlier
			obj.tmp_probability_cost = probability_cost/alpha;
			obj.tmp_distance_cost = distance_cost/beta;
			obj.tmp_heading_cost = heading_cost/gamma;
			obj.tmp_repField_cost = prob_potential_field_cost/zeta;
			obj.tmp_otherAgent_cost = otheragents_cost/10;
			
			% for purpose of cost function plots, rescale back to [0,1] the
			% unreachable components
			obj.tmp_cost = unreachable_cost/10 + ...
                        probability_cost + ...
                        distance_cost + ...
                        heading_cost + ...
        ...
        function [obj] = integratePosition(obj, cumulation, dt)
			...
			vert_distance_error					= 0.18*(obj.z_setpoint(cumulation) - obj.z(cumulation-1)); % the 0.18 factor helps prevent overshoots in approaching the setpoints (by only approaching fractional distances towards the setpoint)
			...
			% Repeat constraints for vertical dimension, being careful of
			% sign/direction
			if vert_distance_error >= 0 % i.e. below setpoint height
				obj.vertical_velocity_setpoint(cumulation) = min(obj.vertPos_p*vert_distance_error,obj.max_vert_velocity);
			else
				% we are above or at the setpoint height
				obj.vertical_velocity_setpoint(cumulation)   = max(obj.vertPos_p*vert_distance_error, -obj.max_vert_velocity);
			end
...
% 			fprintf('Setpoints x: %.2f, y: %.2f, z: %.2f\n', x_set, y_set, obj.z_setpoint(cumulation));
			distance_change = sqrt((x_set - agent_x_current)^2 + (y_set - agent_y_current)^2 + (obj.z_setpoint(cumulation) - obj.z(cumulation-1))^2);
% 			distance_change = euclideanDistance(x_set,y_set,agent_x_current,agent_y_current);
			...
			if yaw_change > yaw_buff && distance_change < dist_buff %abs(new_x_setpoint - agent_x) < osc_buff || abs(new_y_setpoint - agent_y) < osc_buff
				...
				obj.yaw_setpoint(cumulation) = obj.yaw(cumulation-1);
				obj.state = "STANDBY";
			else
